...
- (void)setupScene {
    // Create new scene instead of cleaning up old one
...
- (void)createModelNode {
    ...
    NSLog(@"Creating model node from path: %@", self.modelPath);
    
    if (!self.modelPath) {
        NSLog(@"Error: modelPath is nil");
        return;
    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        SCNNode *newNode = [self loadSTLWithPath:self.modelPath];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!newNode) {
                NSLog(@"Failed to load STL model from path: %@", self.modelPath);
                return;
            }
            
            modelNode = newNode;
            NSLog(@"Model node loaded successfully");
