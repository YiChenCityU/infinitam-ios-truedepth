...
void ITMViewBuilder_CUDA::UpdateView(ITMView **view_ptr, ITMUChar4Image *rgbImage, ITMShortImage *rawDepthImage, bool useBilateralFilter, bool modelSensorNoise)
{
	...
	float *depthData = view->depth->GetData(MEMORYDEVICE_CUDA);
	...
	limitDepthRange_device << <gridSize, blockSize >> > (depthData, imgSize);
...
__global__ void limitDepthRange_device(float *depthData, Vector2i imgSize)
{
	...
	float depth = depthData[x + y * imgSize.x];
	if (depth < 0.4f) depth = 0.4f;
	if (depth > 10.0f) depth = 10.0f;
	depthData[x + y * imgSize.x] = depth;
}
